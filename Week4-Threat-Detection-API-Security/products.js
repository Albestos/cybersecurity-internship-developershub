...
router.get('/', function(req, res, next) {
   ...
   if (!check_logged(req, res)) { return; }
...
router.get('/products/detail', csrfProtection, function(req, res, next) {
    if (!check_logged(req, res)) { return; }
    var url_params = url.parse(req.url, true).query;
    var product_id = url_params.id;
    db_products.getProduct(product_id)
        .then(function (data) {
            res.render('product_detail', { product: data, csrfToken: req.csrfToken() });
        })
});
...
router.all('/products/buy', apiKeyAuth, csrfProtection, function(req, res, next) {
   ...
   db_products.purchase(cart)
        ...
        .catch(function (err) {
            console.log(err);
            return res.status(500).json({message: "Error while processing the purchase"});
        });
